...
type RemoteVfx    = { entity: Entity; lifetime: number }
const remoteBullets: RemoteBullet[] = []
const remoteVfxList: RemoteVfx[]    = []
...
export function spawnRemoteVfx(
  pos: { x: number; y: number; z: number },
  rot: { x: number; y: number; z: number; w: number }
) {
  const entity = engine.addEntity()
  ...
  remoteVfxList.push({ entity, lifetime: VFX_LIFE })
}

engine.addSystem((dt: number) => {
  for (let i = remoteVfxList.length - 1; i >= 0; i--) {
    remoteVfxList[i].lifetime -= dt
    if (remoteVfxList[i].lifetime <= 0) {
      engine.removeEntity(remoteVfxList[i].entity)
      remoteVfxList.splice(i, 1)
    }
  }
